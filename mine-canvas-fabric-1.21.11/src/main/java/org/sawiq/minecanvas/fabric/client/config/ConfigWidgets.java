...
final class ConfigWidgets {
    ...
    static void roundedBorder(GuiGraphics graphics, int x, int y, int width, int height,
                              int border, int fill) {
        roundedRect(graphics, x, y, width, height, border);
        if (width > 2 && height > 2) roundedRect(graphics, x + 1, y + 1, width - 2, height - 2, fill);
    }

    static void roundedRect(GuiGraphics graphics, int x, int y, int width, int height, int color) {
    ...
    private static void verticalGradient(GuiGraphics graphics, int x, int y, int width, int height,
                                         int top, int bottom) {
        if (width <= 0 || height <= 0) return;
    ...
    static void sparkle(GuiGraphics graphics, int x, int y, int color) {
        graphics.fill(x + 2, y, x + 3, y + 5, color);
        graphics.fill(x, y + 2, x + 5, y + 3, color);
    }

    static void playGlyph(GuiGraphics graphics, int x, int y, int color) {
    ...
    static void filmFrame(GuiGraphics graphics, int x, int y) {
    ...
    static void mountain(GuiGraphics graphics, int x, int bottom, int width, int top, int color) {
    ...
    static void tree(GuiGraphics graphics, int x, int bottom, int color) {
    ...
    private abstract static class Row extends AbstractWidget {
        ...
        @Override
        protected final void renderWidget(GuiGraphics graphics, int mouseX, int mouseY,
                                          float partialTick) {
        ...
        private void drawLabel(GuiGraphics graphics, Component label, int right) {
            int x = getX() + 28;
            ...
            graphics.drawString(font, text, x, getY() + (getHeight() - font.lineHeight) / 2, TEXT, false);
        }

        abstract int controlLeft();

        abstract void drawControl(GuiGraphics graphics, int mouseX, int mouseY);
    ...
    static final class Toggle extends Row {
        ...
        @Override
        void drawControl(GuiGraphics graphics, int mouseX, int mouseY) {
    ...
    static final class ToggleCycle<T> extends Row {
        ...
        void drawControl(GuiGraphics graphics, int mouseX, int mouseY) {
            ...
            graphics.drawString(font, text, selectorLeft() + 6,
    ...
    static final class Slider extends Row {
        ...
        void drawControl(GuiGraphics graphics, int mouseX, int mouseY) {
            ...
            String text = value + "%";
            graphics.drawString(font, text, boxX + (boxWidth - font.width(text)) / 2,
                    getY() + (getHeight() - font.lineHeight) / 2, TEXT, false);
        }
    ...
    static final class CycleBox<T> extends Row {
        ...
        void drawControl(GuiGraphics graphics, int mouseX, int mouseY) {
            ...
            String text = display.apply(value);
            graphics.drawString(font, text, controlLeft() + 6, getY() + (getHeight() - font.lineHeight) / 2, TEXT, false);
    ...
    static final class Cache extends Row {
        ...
        void drawControl(GuiGraphics graphics, int mouseX, int mouseY) {
            ...
            graphics.drawString(font, text, controlLeft() + (width - font.width(text)) / 2,
    ...
    private static void chevron(GuiGraphics graphics, int x, int y, boolean up, int color) {
        if (up) {
    ...
    private static void drawIcon(GuiGraphics graphics, int x, int y, Icon icon) {
