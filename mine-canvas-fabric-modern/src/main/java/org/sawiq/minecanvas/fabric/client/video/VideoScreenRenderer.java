...
public final class VideoScreenRenderer {
    ...
    public static void init() {
        LevelRenderEvents.COLLECT_SUBMITS.register(VideoScreenRenderer::onCollectSubmits);
    }
    ...
    private static void onCollectSubmits(LevelRenderContext ctx) {
        ...
        for (VideoScreen screen : VideoScreenManager.all()) {
            ...
            drawScreen(collector, matrices, cam, st, screen.textureId());
            if (screen.showsPauseOverlay()) drawPauseOverlay(collector, matrices, cam, st);
        }

        matrices.popPose();
    }

    private static void drawScreen(SubmitNodeCollector collector,
                                   PoseStack matrices,
                                   Vec3 cam,
                                   ScreenState s,
        ...
        collector.submitCustomGeometry(matrices, RenderTypes.entityCutoutZOffset(textureId), (pose, vc) -> {
    ...
    private static void drawPauseOverlay(SubmitNodeCollector collector,
                                          PoseStack matrices,
                                          Vec3 cam,
                                          ScreenState s) {
